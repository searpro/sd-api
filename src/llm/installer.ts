...
const CLI_NAMES = ['llama-server', 'llama-server.exe'];

export interface LlamaInstallerConfig {
  installDir: string;
...
/**
 * Downloads and installs a prebuilt llama.cpp release matching the host
 * OS/arch. The whole archive is extracted in place because llama-server
 * depends on sibling shared libraries (libllama/libggml*).
 *
 * Mirrors src/sd/installer.ts's SdInstaller exactly (see that file for the
 * rationale on whole-archive extraction + atomic staging-dir swap); kept as a
 * separate class rather than generalized since the two release feeds evolve
 * independently. selectAsset() itself is already generic and reused as-is.
 */
export class LlamaInstaller {
  constructor(
    private readonly config: LlamaInstallerConfig,
  ...
  private async fetchRelease(signal?: AbortSignal): Promise<GithubRelease> {
    ...
    if (!res.ok) {
      ...
      throw errors.downloadFailed(
        `Failed to query llama.cpp release "${this.config.releaseTag}": HTTP ${res.status} ${res.statusText}${hint}`,
      );
    }
  ...
  async install(signal?: AbortSignal): Promise<InstallResult> {
    ...
    this.log.info(
      ...
      'selected llama.cpp release asset',
    );
  ...
  async installAsset(
    ...
    const tmpZip = join(tmpdir(), `llama-cpp-${randomUUID()}.zip`);
    this.log.info({ url: asset.browser_download_url }, 'downloading llama.cpp release archive');
    ...
    try {
      ...
      this.log.info({ binaryPath: finalBinary }, 'llama.cpp installed');
